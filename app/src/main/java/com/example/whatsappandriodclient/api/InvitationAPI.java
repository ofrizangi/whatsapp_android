        ...
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    Contact contact1 = new Contact(userID + contact.getContactName(), contact.getContactName(), contact.getContactNickName(), contact.getServer(), userID, "", "");
...
//                    myIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    ...
                    alert.setMessage("could not invite this contact - make sure the user name exists in " +
                            "the server or the server address is correct");
                    alert.create().show();
                }
            }
